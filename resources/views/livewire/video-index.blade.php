<div>
    ...
    <div class="mb-6 flex flex-wrap items-center gap-3">
        <div class="min-w-0 flex-1">
            <input type="search" {{-- .live.debounce: 打鍵のたびに送るが 400ms 待ってまとめる。
                     検索は「入力しながら結果が絞れる」体験にしたいので .live。 --}} wire:model.live.debounce.400ms="query"
                placeholder="タイトル・チャンネル名・タグで検索"
                class="w-1/2 rounded-lg border border-gray-400 px-3 py-2 text-sm
                       focus:border-gray-900 focus:outline-none">
        </div>

        {{-- タグで絞り込み中はチップを出す。× で解除。 --}}
        @if ($tag !== '')
            <button type="button" wire:click="filterByTag(@js($tag))"
    ...
    {{-- ============ 一覧グリッド ============ --}}
    @if ($videos->isEmpty())
        <div class="border border-dashed border-gray-300 py-16 text-center text-sm text-gray-500">
            @if ($query !== '' || $tag !== '')
                条件に一致する動画がありません。
            @else
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            @foreach ($videos as $video)
                ...
                <div wire:key="video-{{ $video->id }}"
                    class="group relative overflow-hidden border border-gray-400 bg-white transition hover:shadow-md">
                    <a href="{{ route('videos.show', $video) }}" wire:navigate class="absolute inset-0 z-10"
                        aria-label="{{ $video->title ?? '動画詳細' }}"></a>
                        ...
                        @if ($video->tags->isNotEmpty())
                            <div class="relative z-20 mt-2 flex flex-wrap gap-1">
                                @foreach ($video->tags->take(3) as $t)
                                    <button type="button" wire:click="filterByTag(@js($t->name))"
                                        class="rounded bg-gray-100 px-1.5 py-0.5 text-[11px] text-gray-600 hover:bg-gray-200">#{{ $t->name }}</button>
                                @endforeach
                                @if ($video->tags->count() > 3)
                                    <span
                                        class="px-1 py-0.5 text-[11px] text-gray-400">+{{ $video->tags->count() - 3 }}</span>
                                @endif
                            </div>
                        @endif
                    </div>
